...
llm = HuggingFacePipeline.from_model_id(model_id="google/flan-t5-small", task="text2text-generation", pipeline_kwargs={"max_new_tokens": 10}, device=0)#, model_kwargs={"temperature":1e-10})
#llm = HuggingFacePipeline.from_model_id(model_id="local_llms/gpt2", task="text2text-generation", device=0)#, model_kwargs={"temperature":1e-10})
...
def run_prediction(data, log=False):
    if log: print("Question:", validation["Data"][10]["Question"])
    prediction = rag_answer(data["question"], build_context(data), log=log)
    if log: print("\nCorrect answer:", validation["Data"][10]["Answer"])
    return prediction
